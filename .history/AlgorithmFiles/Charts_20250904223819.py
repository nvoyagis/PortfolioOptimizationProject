...
def heatmap(M, cmap="viridis", vmin=None, vmax=None, save=None, show=True):
    """
    Plot a heatmap from a dense 2D matrix-like object.
    - cmap can be a Matplotlib colormap name, a Colormap object, or a list of colors.
    - No title. Hides ticks for a clean look.
    """
    A = np.asarray(M)

    # If the user passed a list of colors, build a colormap on the fly
    if isinstance(cmap, (list, tuple)):
        cmap = mcolors.LinearSegmentedColormap.from_list("custom", cmap)

    fig, ax = plt.subplots()
    im = ax.imshow(A, cmap=cmap, aspect="auto", vmin=vmin, vmax=vmax, interpolation="nearest")
