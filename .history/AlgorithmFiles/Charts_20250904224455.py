...
def heatmap_with_labels(M, labels, cmap="viridis", vmin=None, vmax=None, save=None, show=True, rotate=90):
    """
    Plot a heatmap for a square dense matrix with the same string labels on rows & columns.
    - labels: list of strings used for both axes, in order from the top-left.
    - cmap: name, Colormap object, or list of colors (e.g., ["#0d0887","#cc4778","#f0f921"]).
    """
    A = np.asarray(M)
    if A.ndim != 2:
        raise ValueError("M must be 2D.")
    n_rows, n_cols = A.shape
    if len(labels) != n_rows or len(labels) != n_cols:
        raise ValueError(f"labels must match both rows and columns (square matrix). "
                         f"Got len(labels)={len(labels)}, shape={A.shape}")
