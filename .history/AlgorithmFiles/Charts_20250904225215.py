...
def heatmap(M,
            labels=None,            # one list for BOTH axes (optional)
            row_labels=None,        # rows (top→bottom)
            col_labels=None,        # cols (left→right)
            cmap="viridis",
            vmin=None, vmax=None,
            save=None, show=True,
            rotate=90,              # rotation for x tick labels
            fontfamily="STIXGeneral",
            fontsize=10):
    """
    Plot a heatmap for a dense 2D matrix-like object.
    - If `labels` is given, it is used for both rows & columns.
    - Or pass `row_labels` and `col_labels` separately.
    - `cmap` accepts a name, a Colormap, or a list of colors.
    """
    A = np.asarray(M)
    if A.ndim != 2:
    ...
    row_labels = row_labels if row_labels is not None else [str(i) for i in range(n_rows)]
    col_labels = col_labels if col_labels is not None else [str(j) for j in range(n_cols)]

    if len(row_labels) != n_rows or len(col_labels) != n_cols:
        raise ValueError(f"Label lengths must match matrix shape. "
                         f"Got len(row_labels)={len(row_labels)}, len(col_labels)={len(col_labels)}, "
                         f"shape={A.shape}")

    # Build a custom colormap on the fly if a list of colors is provided
    if isinstance(cmap, (list, tuple)):
        cmap = mcolors.LinearSegmentedColormap.from_list("custom", cmap)

    fig, ax = plt.subplots()
    im = ax.imshow(A, cmap=cmap, vmin=vmin, vmax=vmax, aspect="auto", origin="upper")
    fig.colorbar(im, ax=ax)

    # Ticks & labels (modern API with labels=)
    ax.set_xticks(np.arange(n_cols), labels=col_labels)
    ax.set_yticks(np.arange(n_rows), labels=row_labels)

    # Put column labels on top (start at top-left)
    ax.tick_params(top=True, bottom=False, labeltop=True, labelbottom=False)

    # Rotation & font for ticks
    if rotate:
