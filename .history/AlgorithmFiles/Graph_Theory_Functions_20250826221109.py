...
def get_weight_mat(stock_list: list[str], begin_data_date: str, cutoff: str):
    ...
    upper_cov_mat = 0.0001 + (0.9999 - 0.0001) * ((upper_cov_mat - min_cov)/(max_cov - min_cov)) # Normalize covariance to be between [1/10000, 9999/10000] NOTE: Include to make a TMFG of weak relationships
    cov_mat = 0.0001 + (0.9999 - 0.0001) * ((cov_mat - np.min(cov_mat))/(np.max(cov_mat) - np.min(cov_mat))) # Normalize covariance to be between [1/1000, 999/1000] NOTE: Include to make a TMFG of weak relationships
