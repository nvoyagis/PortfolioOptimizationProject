...
def svm_classifier(df: pd.DataFrame, test_size=0.25, plot_cm=True):
    ...
    x = df.drop(columns=['SPX_win_percentage', 'average_return', 'portfolio', 'id', 'TMFG_vert_weight_sum', 'TMFG_edge_weight_sum', 'katz', 'degree', 'eigenvector']) # Remove unecessary features and TMFG-related features (b.c. TMFG-related features show up in the nodes/edges of dual TMFG features)
    ...
    def _explain(best_model, Xtr, Xte, label, feature_names):
        ...
        explainer = shap.Explainer(f, X_bg)  # KernelExplainer under the hood
        ...
        feature_labels = {
            ...
            'communicablity_betweenness': 'Communicability Betweenness Centrality',
        ...
        my_cmap = LinearSegmentedColormap.from_list('my_cmap', ['#a0bec8', '#de8585'])
...
def svm_optimizer(df_list1, df_list2, test_size=0.25):
    ...
    for df in df_list2:
        x = x = df.drop(columns=['SPX_win_percentage', 'average_return', 'portfolio', 'id', 'TMFG_vert_weight_sum', 'TMFG_edge_weight_sum', 'katz', 'degree', 'eigenvector'])
        scaler = sk.preprocessing.MinMaxScaler()
        x = pd.DataFrame(scaler.fit_transform(x), columns=x.columns)
        y1, y2 = df['SPX win percentage'], df['average return']
        y1 = (y1 > 0.5).astype(int)
