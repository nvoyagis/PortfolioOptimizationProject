...
def svm_classifier(df: pd.DataFrame, test_size=0.25, plot_cm=True):
    ...
    def _explain(best_model, Xtr, Xte, label, feature_names):
        ...
        shap.summary_plot(shap_values, X_explain, cmap=my_cmap, feature_names=feature_names, show=False)
        plt.title(f"SHAP Summary")
        plt.xlabel("SHAP Value (Impact On Model Output)")

        # Grab current figure and all axes
        fig = plt.gcf()
        axes = fig.get_axes()
        main_ax, cbar_ax = axes[0], axes[1]
