...
def example_rbf_svm(df: pd.DataFrame,
    ...
    model = sklearn.Pipeline([
        ...
        ("svc", sklearn.SVC(kernel="rbf", C=C, gamma=gamma, class_weight=class_weight, probability=False))
    ])
    model.fit(X, y)
    ...
    ax.contourf(xx, yy, Z, alpha=0.25, levels=[-0.5,0.5,1.5], colors=["#87cefa","#fa8072"])  # blue/red regions
    # scatter points
    ax.scatter(X[y==0,0], X[y==0,1], s=25, edgecolor="k", facecolor="#1f77b4", label="label 0")
    ax.scatter(X[y==1,0], X[y==1,1], s=25, edgecolor="k", facecolor="#d62728", label="label 1")
