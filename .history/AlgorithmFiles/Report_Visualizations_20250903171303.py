...
def example_rbf_svm(df: pd.DataFrame,
    ...
    ----------
    ...
    y_stretch : float, factor to stretch vertical axis (default=1.0 = no stretch)
    """
    ...
    param_grid = {
        ...
        'svc__kernel': ['rbf'],
        'svc__degree': [1],
        'svc__coef0':  [-4, -3, -2, -1, 0, 1, 2, 3, 5],
    }

    gs = sklearn.model_selection.GridSearchCV(pipe, param_grid=param_grid, scoring=scoring, cv=cv,
    ...
    # Apply y_stretch by expanding vertical range
    center_y = (y_max + y_min) / 2
    half_range_y = (y_max - y_min) / 2 * y_stretch
    y_min = center_y - half_range_y
    y_max = center_y + half_range_y
    ...
    ax.contourf(xx, yy, Z, alpha=0.40,
                levels=[-0.5,0.5,1.5],
                colors=["#a0bec8","#de8585"])
    ax.scatter(X[y==0,0], X[y==0,1], s=25, edgecolor="k",
               facecolor="#de8585", label="Unsuccessful Portfolio")
    ax.scatter(X[y==1,0], X[y==1,1], s=25, edgecolor="k",
...
print(df2)
example_rbf_svm(df2, y_stretch=1.5)