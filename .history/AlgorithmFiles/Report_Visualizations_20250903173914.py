...
def example_rbf_svm(df: pd.DataFrame,
    ...
    # scatter points
    ax.scatter(X[y==0,0], X[y==0,1], s=70, edgecolor="k",
               facecolor="#de8585", label="Unsuccessful Portfolio")
    ax.scatter(X[y==1,0], X[y==1,1], s=70, edgecolor="k",
               facecolor="crimson", label="Successful Portfolio")

    ax.set_xlabel('Historical Percent Change')
    ax.set_ylabel('Second Order Centrality')
    ax.set_title('SVC Using an Optimized Gaussian Kernel')
    ax.legend(loc="best")
...
df2 = Database.get_cols('stocks:30 1:2024-01-02 2:2024-01-31 3:2024-02-01 4:2024-02-29', ['vert_weight', 'current_flow', 'SPX_win_percentage'])
print(df2)
example_rbf_svm(df2)