...
def simulate(sims: int, seed: int, stocks: list[str], begin_data_date: str, buy_date: str, sell1: str, sell2: str, analyze_node_features = False, frequently_appearing_check = False):
    ...
    with open(os.path.join('Simulations', f'{sims}_Sims2_{begin_data_date}_to_{buy_date}_and_{sell1}_to_{sell2}.txt'), 'w', newline='') as f:
        ...
        # plt.title('TMFG')
        pos0 = nx.planar_layout(g, scale=10)
        is_planar, emb = nx.check_planarity(g, counterexample=False)
        outer = list(emb.outer_face())              # outer face vertices in order

        # Put outer face on a big circle; fix them there
        R = 5.0
        fixed_pos = {v: (R*np.cos(2*np.pi*i/len(outer)),
                        R*np.sin(2*np.pi*i/len(outer)))
                    for i, v in enumerate(outer)}

        pos0 = nx.tutte_layout(g, pos=fixed_pos, fixed=outer)  # planar & nicely spread
...
simulate(100, 1, stocks3, '2023-01-03', '2023-01-31', '2023-02-01', '2023-02-28') 
